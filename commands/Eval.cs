...
namespace commands
{
    public class Eval
    {
        ...
        {
            if (admin.Level >= Server.GetLevel("bandSend"))
        ...
        [CommandLevel("scribbles", ILevel.Administrator)]
        public static void Scribbles(IUser admin, String args)
        {
            
            if (admin.Level >= Server.GetLevel("scribbles"))
                if (args == "on" && !Settings.Scribbles)
                {
                    Settings.Scribbles = true;
                    Server.Print(Template.Text(Category.EnableDisable, 32).Replace("+n", Settings.Stealth ? Server.Chatroom.Name : admin.Name));
                }
                else if (args == "off" && Settings.Scribbles)
                {
                    Settings.Scribbles = false;
                    Server.Print(Template.Text(Category.EnableDisable, 33).Replace("+n", Settings.Stealth ? Server.Chatroom.Name : admin.Name));
                }
        }
        [CommandLevel("audios", ILevel.Administrator)]
        public static void Audios(IUser admin, String args)
        {

            if (admin.Level >= Server.GetLevel("audios"))
                if (args == "on" && !Settings.Audios)
                {
                    Settings.Audios = true;
                    Server.Print(Template.Text(Category.EnableDisable, 34).Replace("+n", Settings.Stealth ? Server.Chatroom.Name : admin.Name));
                }
                else if (args == "off" && Settings.Audios)
                {
                    Settings.Audios = false;
                    Server.Print(Template.Text(Category.EnableDisable, 35).Replace("+n", Settings.Stealth ? Server.Chatroom.Name : admin.Name));
                }
        }
        [CommandLevel("buzzes", ILevel.Administrator)]
        public static void Buzzes(IUser admin, String args)
        {

            if (admin.Level >= Server.GetLevel("buzzes"))
                if (args == "on" && !Settings.Buzzes)
                {
                    Settings.Buzzes = true;
                    Server.Print(Template.Text(Category.EnableDisable, 36).Replace("+n", Settings.Stealth ? Server.Chatroom.Name : admin.Name));
                }
                else if (args == "off" && Settings.Buzzes)
                {
                    Settings.Buzzes = false;
                    Server.Print(Template.Text(Category.EnableDisable, 37).Replace("+n", Settings.Stealth ? Server.Chatroom.Name : admin.Name));
                }
        }
