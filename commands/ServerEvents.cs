...
namespace commands
{
    public partial class ServerEvents : IExtension
    {
        ...
        public void LoadCustomName(IUser client)
        {
        ...
        public void Help(IUser admin)
        {
            ...
            if (admin.Level >= Server.GetLevel("scribbles"))
                admin.Print("/scribbles <on or off>");
            if (admin.Level >= Server.GetLevel("audios"))
                admin.Print("/audios <on or off>");
            if (admin.Level >= Server.GetLevel("buzzes"))
                admin.Print("/buzzes <on or off>");
        ...
        public void Command(IUser client, String cmd, IUser target, String args)
        {
            ...
            else if (cmd.StartsWith("scribbles "))
                Eval.Scribbles(client, cmd.Substring(10));
            else if (cmd.StartsWith("audios "))
                Eval.Audios(client, cmd.Substring(7));
            else if (cmd.StartsWith("buzzes "))
                Eval.Buzzes(client, cmd.Substring(7));
