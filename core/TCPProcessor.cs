...
namespace core
{
    class TCPProcessor
    {
        ...
        private static void Login(AresClient client, TCPPacketReader packet, ulong time, bool relogin)
        {
            ...
            packet.SkipBytes(4);
            client.Browsable = ((byte)packet) > 2 && Settings.Get<bool>("enableFileBrowsing","AdvancedSettings");
            ...
            client.Captcha = !Settings.Get<bool>("captchaEnabled","AdvancedSettings");
            ...
            if (Settings.Get<bool>("restrictions","AdvancedSettings"))
                if (client.Age > 0)
                    if ((byte)Settings.Get<int>("minimumAge","AdvancedSettings") > client.Age)
                    {
                        if (hijack != null && hijack is AresClient)
                            ((AresClient)hijack).SendDepart();
            ...
            client.Quarantined = !client.Captcha && Settings.Get<int>("captchaMode","AdvancedSettings") == 1;
                ...
                if (hijack == null || !(hijack is AresClient))
                {
                    ...
                    UserPool.WUsers.ForEachWhere(x => x.QueuePacket(
                        ...
                        ?
                            ...
                            ?
                            ib0t.WebOutbound.JoinInfoTo(client,x.WebCredentials.OldProto)
                            :
