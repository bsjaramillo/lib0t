...
namespace core
{
    public class ServerCore
    {
        ...
        public bool Open()
        {
            ...
            this.udp = new UdpListener(new IPEndPoint(new IPAddress(IPAddress.Parse(Settings.Get<string>("UDPHostAddress", "AdvancedSettings")).GetAddressBytes()), Settings.Port));
        ...
        private void ServerThread()
        {
            ...
            if (Settings.Get<bool>("enableRoomSearch","AdvancedSettings"))
            ...
            bool can_web_chat = Settings.Get<bool>("ib0tSupportEnabled", "MainSettings");
            core.LinkHub.LinkMode link_mode = (core.LinkHub.LinkMode) Settings.Get<int>("linkMode", "LinkingSettings");
        ...
        private void ServiceAresSockets(ulong time)
        {
            foreach (AresClient client in UserPool.AUsers)
                if (!String.IsNullOrEmpty(client.DNS))
                {
                    if (client.IsHTML)
                    {
                        if (Settings.Get<bool>("ib0tSupportEnabled", "MainSettings"))
                    ...
                    {
                        {
                            ...
                            UserPool.AUsers.ForEachWhere(x =>
                            {
                                ...
                                {
                                    x.Print(message);
                                    x.PM(Settings.Get<String>("botName","MainSettings"), message);
                                }
